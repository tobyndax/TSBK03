...
enum {kNumBalls = 1}; // Change as desired, max 16
...
void loadModelTexturePair(ModelTexturePair* modelTexturePair,char* model, char* texture)
{
...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
	{
		...
		//		v := P * 1/mass
		if(ball[i].X.y > 0.1 + kBallSize){
			ball[i].P.y = ball[i].mass*(ball[i].v.y-9.8f*deltaT);
		}
		else{
			r = VectorSub(ball[i].X, SetVector(ball[i].X.x,0,ball[i].X.z));
			imp = -(ELASTICITY + 1)*DotProduct(Normalize(r),VectorSub(ball[i].v, SetVector(0,0,0)))/(1.0/ball[i].mass);
			ball[i].P.y = VectorAdd(SetVector(0,0,0),VectorAdd(ScalarMult(Normalize(r), imp*0.90f), ball[i].P)).y; //impulse in direction of n, added to P.
		}
...
void renderBall(int ballNr)
{
	...
	GLfloat scale = 2.0f;
	// Ball with rotation
	transMatrix = T(ball[ballNr].X.x, kBallSize + ball[ballNr].X.y, ball[ballNr].X.z); // position
	scaleMatrix = S(scale,scale,scale);
	tmpMatrix = Mult(transMatrix, ball[ballNr].R); // ball rotation
	tmpMatrix = Mult(tmpMatrix,scaleMatrix);
	...
	tmpMatrix = S(scale*0.75*(1+ball[ballNr].X.y), 0.0, scale*0.75*(1+ball[ballNr].X.y));
